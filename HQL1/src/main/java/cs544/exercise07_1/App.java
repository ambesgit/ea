...
public class App {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // TODO update HQL
            @SuppressWarnings("unchecked")
            List<Airline> airlines = session.createQuery("select distinct ap from Airline ap join ap.flights f where f.airplane.model='A380'").list();
        ...
        try {
            ...
            // TODO update HQL
            @SuppressWarnings("unchecked")
            List<Flight> flights = session.createQuery("select f from Flight f join"+
                    " f.airplane ap join f.airline al where ap.model=747 and al.name!='Star Alliance'").list();
            
            System.out.println("All flights using 747 planes that don't belong to 'Star Alliance'"); 
            System.out.println("Flight:  Departs:     "
                    + "                  Arrives:");
            for (Flight flight : flights) {
                System.out.printf("%-7s  %-12s %7s %8s  %-12s %7s %8s\n",
        ...
        try {
            ...
            // TODO update HQL
            @SuppressWarnings("unchecked")
            List<Flight> flights = session.createQuery("select f from Flight f where f.departureDate='2009-08-07' and f.departureTime<'23:59:59'").list();
    ...
    public static void solutionsOfHQL1(Session session, Transaction tx,String[] a_dHQL1){
        ...
        try{
            ...
            int x;            
            for(x=0; x<a_dHQL1.length;x++){
            switch(x){
                case 0:
                    query=session.createQuery(a_dHQL1[x]);
                    flights=query.list();
                     System.out.println(a_dHQL1[x]);
            ...
            if(tx!=null){
                System.out.println("roll back transaction"+e.getMessage());
                tx.rollback();
            }
